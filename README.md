# Hi there, I'm Khang! 👋 

### About Me
- 🎓 **Senior Student** specializing in **Artificial Intelligence & Machine Learning**.
- Currently on an intensive journey to master Data Science and Model Engineering.
- Actively seeking an **AI/ML Internship**.
- **English Proficiency:** TOEIC 840 (Ready for international documentation and collaboration).

---

### Machine Learning Stack
- **Languages:** Python (Primary), SQL.
- **Libraries:** Scikit-learn, Pandas, NumPy, Matplotlib, Seaborn, XGBoost, Optuna, Time Series.
- **Algorithms:** Linear/Logistic Regression, Decision Trees, Random Forest, SVM, Clustering.
- **Concepts:** Feature Engineering, Data Cleaning, Model Evaluation (MSE, R2, F1-Score), Gradient Descent.
- **Tools:** Git, GitHub, Google Colab, VS Code.

---

### Let's Connect:
- **LinkedIn:** Don't have it at the moment
- **Email:** khangtranttns@gmailgmail.com

<p align="center">
  <img src="https://github-readme-stats.
